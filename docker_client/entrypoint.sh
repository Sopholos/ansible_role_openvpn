...
# Debugging
echo "
$TIMESTAMP -- VPN Configuration ---
"

echo "$TIMESTAMP oVPN configuration file: $VPN_OVPN"

echo "$TIMESTAMP VPN auth file: $VPN_AUTH_FILE"

# Verify the vpn service file exists
if [ ! -d "$VPN_SERVICE_FOLDER" ]; then
	echo "$TIMESTAMP Invalid VPN service: $VPN_SERVICE"
	exit
fi

# Verify the vpn config file exists
if [ ! -f "$VPN_OVPN" ]; then
	echo "$TIMESTAMP Unable to find VPN server: $VPN_SERVER"
	exit
fi
